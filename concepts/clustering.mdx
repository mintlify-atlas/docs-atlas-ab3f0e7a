---
title: "Spatio-Temporal Event Clustering with DBSCAN"
sidebarTitle: "Clustering"
description: "How GeoSentinel's DBSCAN algorithm groups nearby concurrent events from multiple sources into single deduplicated incidents using a normalized distance metric."
---
...
- **FIRMS** detects a thermal hotspot at (36.82°N, 36.44°E) via MODIS satellite.
- **GDELT** extracts a news article placing the same fire at (36.80°N, 36.46°E).
- **ACLED** logs a related displacement event at (36.83°N, 36.43°E).

All three events have `category = "wildfire"` and are within 4 km and 6 hours of each other. Without clustering, the API returns three separate incidents. With clustering, they collapse into one incident whose `canonical_point` is the confidence-weighted centroid of the three observations, `source_count = 3`, and `confidence` is boosted by the sensor observation (FIRMS has independence class `sensor`, weight ×2.0).
